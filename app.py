...
def load_league_settings() -> dict:
    ...
    """
    fallback = {
        "PL": (-0.13, 0.4), "PD": (-0.18, 0.6), "BL1": (-0.08, 0.6),
        "SA": (-0.18, 0.4), "FL1": (-0.18, 0.2), "DED": (0.00, 0.8),
        "PPL": (-0.18, 0.4), "ELC": (-0.13, 0.6), "BSA": (-0.18, 0.6),
        "CL": (-0.13, 0.4),
    }
    try:
...
def compute_team_over_under(matrix: dict, is_home: bool, seuils=(0.5, 1.5, 2.5)) -> dict:
    """
    Calcule les probabilités Over/Under de buts marqués pour une équipe,
    en marginalisant la matrice jointe Dixon-Coles déjà utilisée pour le
    score exact (donc cohérent avec le reste du modèle, pas une loi de
    Poisson indépendante recalculée à côté).

    is_home=True  -> buts de l'équipe domicile (indice h du tuple)
    is_home=False -> buts de l'équipe extérieure (indice a du tuple)
    """
    resultats = {}
    for seuil in seuils:
        borne = int(seuil)  # ex. 1.5 -> 1 (Over 1.5 = marquer 2 buts ou plus)
        if is_home:
            p_under_ou_egal = sum(p for (h, a), p in matrix.items() if h <= borne)
        else:
            p_under_ou_egal = sum(p for (h, a), p in matrix.items() if a <= borne)
        resultats[seuil] = 1 - p_under_ou_egal
    return resultats
...
def run_prediction_for_teams(team1: dict, team2: dict) -> dict:
    ...
    home_goals_ou = compute_team_over_under(matrix, is_home=True)
    away_goals_ou = compute_team_over_under(matrix, is_home=False)
    ...
    return {
        ...
        "double_chance_1x": home_win + draw, "double_chance_x2": away_win + draw,
        "double_chance_12": home_win + away_win,
        "home_goals_ou": home_goals_ou, "away_goals_ou": away_goals_ou,
...
def render_prediction(result: dict, subtitle: str | None = None) -> None:
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric(f"1X ({t1['name']} ou nul)", f"{result['double_chance_1x'] * 100:.1f}%")
    c2.metric("12 (pas de nul)", f"{result['double_chance_12'] * 100:.1f}%")
    c3.metric(f"X2 (nul ou {t2['name']})", f"{result['double_chance_x2'] * 100:.1f}%")
    ...
    st.markdown("### Buts marqués par équipe (Over/Under)")
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"**{t1['name']} (domicile)**")
        for seuil, proba in result["home_goals_ou"].items():
            st.write(f"Over {seuil} : {proba * 100:.1f}%")
    with c2:
        st.markdown(f"**{t2['name']} (extérieur)**")
        for seuil, proba in result["away_goals_ou"].items():
            st.write(f"Over {seuil} : {proba * 100:.1f}%")
    if t1["competition_code"] == "BSA" or t2["competition_code"] == "BSA":
        st.caption("⚠️ Marché moins fiable sur le Brasileirão d'après le backtesting "
                   "(n'apporte pas d'avantage net par rapport à la fréquence historique).")
